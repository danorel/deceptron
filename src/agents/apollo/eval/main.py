...
Mirrors `agents.deceptron.eval.main` exactly — same filter, same metrics
(`shared.metrics.compute_pauroc` / `bootstrap_ci`), same dual cutoffs
(deceptron's FPR<0.05 and Apollo's FPR<0.20), same bootstrap 95% CIs — so
results from the two pipelines are directly comparable (see `src/benchmark/`
for the side-by-side report against Apollo's reported numbers).

Reads scores.jsonl (output of agents.apollo.models.monitor.prompted.main, one file per Apollo
reference prompt variant) and writes results.json.

Usage:
    python -m agents.apollo.eval.main --scores data/agents/apollo/scores/human_refined_gloom.jsonl \
        --out data/agents/apollo/results/human_refined_gloom/
"""
...
from shared.metrics import APOLLO_MAX_FPR, bootstrap_ci, compute_pauroc


def run(scores: str, max_fpr: float, out: str | None) -> None:
    scores_path = Path(scores)
    if not scores_path.exists():
        sys.exit(f"{scores} not found")
    ...
    pauroc = compute_pauroc(y_true, y_score, max_fpr)
    ...
    pauroc_lo, pauroc_hi = bootstrap_ci(
        y_true, y_score, lambda yt, ys: compute_pauroc(yt, ys, max_fpr))
    ...
    print(f"pAUROC (FPR<{max_fpr:.2f}):     {pauroc:.4f}  95% CI [{pauroc_lo:.3f}, {pauroc_hi:.3f}]")
...
def main() -> None:
    ...
    parser.add_argument("--scores", required=True,
                        help="Path to one prompt-variant scores.jsonl (output of agents.apollo.models.monitor.prompted.main)")
    ...
    run(args.scores, args.max_fpr, args.out)
